 ...
 * CSVフォーマット: areaCode,year,value（valueは単位なし）
 * 想定運用: data/raw/bmi.csv を最新のデータに差し替えて npm run sync。
 */
export const manualBmiProvider: Provider = {
  id: "manual-bmi",
  datasetId: "bmi",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "bmi.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/bmi] data/raw/bmi.csv が無いためモックデータを使用します");
      return mockBmi();
    }
    return parseCsv(raw);
  }

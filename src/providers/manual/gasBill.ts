 ...
 * 想定運用: data/raw/gasBill.csv を最新のデータに差し替えて npm run sync。
 */
export const manualGasBillProvider: Provider = {
  id: "manual-gas-bill",
  datasetId: "gasBill",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "gasBill.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/gasBill] data/raw/gasBill.csv が無いためモックデータを使用します");
      return mockGasBill();
    }
    return parseCsv(raw);
  }
...
function mockGasBill(): DataPoint[] {

...
/**
 * 総務省統計局「小売物価統計調査」（2025年、都道府県庁所在市等）
 * CSVフォーマット: areaCode,year,value（valueは円）
 * 想定運用: data/raw/barberFee.csv を最新のデータに差し替えて npm run sync。
 */
export const manualBarberFeeProvider: Provider = {
  id: "manual-barber-fee",
  datasetId: "barberFee",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "barberFee.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/barberFee] data/raw/barberFee.csv が無いためモックデータを使用します");
      return mockBarberFee();
    }
    return parseCsv(raw);
  }
};
...
function mockBarberFee(): DataPoint[] {

 ...
 * 想定運用: data/raw/bananaPrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualBananaPriceProvider: Provider = {
  id: "manual-banana-price",
  datasetId: "bananaPrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "bananaPrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/bananaPrice] data/raw/bananaPrice.csv が無いためモックデータを使用します");
      return mockBananaPrice();
    }
    return parseCsv(raw);
  }
};
...
function mockBananaPrice(): DataPoint[] {

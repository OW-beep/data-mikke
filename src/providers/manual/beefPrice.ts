 ...
 * 想定運用: data/raw/beefPrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualBeefPriceProvider: Provider = {
  id: "manual-beef-price",
  datasetId: "beefPrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "beefPrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/beefPrice] data/raw/beefPrice.csv が無いためモックデータを使用します");
      return mockBeefPrice();
    }
    return parseCsv(raw);
  }
};
...
function mockBeefPrice(): DataPoint[] {

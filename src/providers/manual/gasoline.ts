 ...
 * CSVフォーマット: areaCode,year,value（valueは円/L）
 * 想定運用: data/raw/gasoline.csv を最新のデータに差し替えて npm run sync。
 */
export const manualGasolineProvider: Provider = {
  id: "manual-gasoline",
  datasetId: "gasoline",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "gasoline.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/gasoline] data/raw/gasoline.csv が無いためモックデータを使用します");
      return mockGasoline();
    }
    return parseCsv(raw);
  }
...
function mockGasoline(): DataPoint[] {

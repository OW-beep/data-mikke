 ...
 * 想定運用: data/raw/eggPrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualEggPriceProvider: Provider = {
  id: "manual-egg-price",
  datasetId: "eggPrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "eggPrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/eggPrice] data/raw/eggPrice.csv が無いためモックデータを使用します");
      return mockEggPrice();
    }
    return parseCsv(raw);
  }
...
function mockEggPrice(): DataPoint[] {

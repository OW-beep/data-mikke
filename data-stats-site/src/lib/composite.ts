 ...
 * 全国平均と比べて「良い方向」にあるかどうかを数えるだけの、シンプルな独自集計。
...
export function compositeComment(score: number, total: number): string {
  ...
  if (ratio >= 0.8) {
    return "5つの指標のうち多くで全国平均を上回っており、経済・生活基盤の面で総合力の高い都道府県といえます。";
  }
  if (ratio >= 0.6) {
    return "全国平均を上回る指標がやや多く、経済・生活基盤の面でバランスの取れた都道府県です。";
  }
  if (ratio >= 0.4) {
    return "全国平均を上回る指標と下回る指標がほぼ半々で、指標によって強み・弱みが分かれています。";
  }
  return "5つの指標のうち多くで全国平均を下回っており、経済・生活基盤の面では課題を抱えやすい都道府県といえます。";
}

...
export const culturalPropertyFourWaysToSeeIt: Article = {
  ...
  title: "国宝・重要文化財数ランキング、総数1位は東京都2,729件。でも「数え方」を変えると1位が3回入れ替わる",
  excerpt:
    "都道府県別の国宝・重要文化財数ランキングは、総数なら東京都(2,729件)・京都府(2,144件)、建造物だけなら京都府、人口あたりなら奈良県・滋賀県が1位に浮かび上がります。同じ統計でも数え方次第で1位が変わる理由を、実際の件数で解説します。",
  ...
  body: [
    "京都や奈良を旅行すると、街のあちこちに国宝級の寺社仏閣があることに驚かされます。この感覚は数字にもそのまま表れていて、都道府県別の国宝・重要文化財数ランキングを見ると、1位東京都(2,729件)、2位京都府(2,144件)、3位奈良県(1,311件)、4位滋賀県(813件)と、この上位4都府県だけで、全国の半数近くを占めています。ただし、この「総数」という数え方は、実は文化財の実態をとらえる4つの切り口のうち、ほんの1つに過ぎません。数え方を変えるたびに、1位の顔ぶれが入れ替わっていく——それが、この統計の一番面白いところです。",

    "【切り口1: 総数】特に東京都(2,729件)と京都府(2,144件)は2,000件を超え、3位の奈良県(1,311件)以下を大きく引き離しています。ただし、この2都府県が上位に来る理由はまったく異なります。京都府・奈良県・滋賀県は、長きにわたって都が置かれ、寺社仏閣や仏像・絵画などの文化財が長い年月をかけて蓄積されてきた結果です。特に奈良県は法隆寺や東大寺をはじめとする古代の建造物が数多く現存しています。一方、東京都が総数でトップなのは「美術工芸品」（絵画・書跡・刀剣など、博物館に収蔵される移動可能な文化財）の指定件数が突出して多いためで、明治以降の首都機能の集中にともない、全国の博物館・皇室関連の収蔵品が集められてきた経緯によるものです。つまり東京都の高さは、近代以降に「集められた」結果であり、京都・奈良のように古くからその土地にあった蓄積とは性質が異なります。",

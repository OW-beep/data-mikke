 ...
 * 想定運用: data/raw/cabbagePrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualCabbagePriceProvider: Provider = {
  id: "manual-cabbage-price",
  datasetId: "cabbagePrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "cabbagePrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/cabbagePrice] data/raw/cabbagePrice.csv が無いためモックデータを使用します");
      return mockCabbagePrice();
    }
    return parseCsv(raw);
  }
};
...
function mockCabbagePrice(): DataPoint[] {

 ...
 * 想定運用: data/raw/breadPrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualBreadPriceProvider: Provider = {
  id: "manual-bread-price",
  datasetId: "breadPrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "breadPrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/breadPrice] data/raw/breadPrice.csv が無いためモックデータを使用します");
      return mockBreadPrice();
    }
    return parseCsv(raw);
  }
};
...
function mockBreadPrice(): DataPoint[] {

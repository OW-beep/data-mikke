 ...
 * 想定運用: data/raw/cleaningFee.csv を最新のデータに差し替えて npm run sync。
 */
export const manualCleaningFeeProvider: Provider = {
  id: "manual-cleaning-fee",
  datasetId: "cleaningFee",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "cleaningFee.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/cleaningFee] data/raw/cleaningFee.csv が無いためモックデータを使用します");
      return mockCleaningFee();
    }
    return parseCsv(raw);
  }
};
...
function mockCleaningFee(): DataPoint[] {

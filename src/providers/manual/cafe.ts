...
/**
 * 喫茶店の軒数CSVを読み込むProvider。
 * 想定運用: data/raw/cafe.csv を最新の集計に差し替えて npm run sync。
 */
export const manualCafeProvider: Provider = {
  id: "manual-cafe",
  datasetId: "cafe",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "cafe.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/cafe] data/raw/cafe.csv が無いためモックデータを使用します");
      return mockData();
    }
    return parseCsv(raw);
  }
...
function mockData(): DataPoint[] {
  return PREFECTURES.map((pref) => ({
    ...
    areaName: pref.name,
    year: 2008,
    value: 1000 + (Number(pref.code) % 15) * 700
  }));
}

...
export const manualMichinoekiProvider: Provider = {
  id: "manual-michinoeki",
  datasetId: "michinoeki",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "michinoeki.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/michinoeki] data/raw/michinoeki.csv が無いためモックデータを使用します");
      return mockData();
    }
    return parseCsv(raw);
  }

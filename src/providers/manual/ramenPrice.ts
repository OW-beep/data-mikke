 ...
 * 想定運用: data/raw/ramenPrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualRamenPriceProvider: Provider = {
  id: "manual-ramen-price",
  datasetId: "ramenPrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "ramenPrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/ramenPrice] data/raw/ramenPrice.csv が無いためモックデータを使用します");
      return mockRamenPrice();
    }
    return parseCsv(raw);
  }
};
...
function mockRamenPrice(): DataPoint[] {

 ...
 * 想定運用: data/raw/porkPrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualPorkPriceProvider: Provider = {
  id: "manual-pork-price",
  datasetId: "porkPrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "porkPrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/porkPrice] data/raw/porkPrice.csv が無いためモックデータを使用します");
      return mockPorkPrice();
    }
    return parseCsv(raw);
  }
};
...
function mockPorkPrice(): DataPoint[] {

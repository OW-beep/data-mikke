 ...
 * 想定運用: data/raw/milkPrice.csv を最新のデータに差し替えて npm run sync。
 */
export const manualMilkPriceProvider: Provider = {
  id: "manual-milk-price",
  datasetId: "milkPrice",

  async fetch(): Promise<DataPoint[]> {
    const csvPath = path.join(process.cwd(), "data", "raw", "milkPrice.csv");
    let raw: string;
    try {
      raw = await readFile(csvPath, "utf-8");
    } catch {
      console.warn("[manual/milkPrice] data/raw/milkPrice.csv が無いためモックデータを使用します");
      return mockMilkPrice();
    }
    return parseCsv(raw);
  }
};
...
function mockMilkPrice(): DataPoint[] {
